...
again: ingestion asks a model to transcribe it instead, so a stored value can
disagree with the document's own header. A field the model has to decide
about, such as the date, is where it goes wrong, and it is the field recency
is decided on.
